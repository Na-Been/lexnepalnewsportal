...
class NewsRepo
{
    ...
    public function allNewsByCategorySlug($id)
    {
        return NewsCategory::leftJoin('news', 'news_categories.news_id', '=', 'news.id')
            ...
            ->where('news_categories.category_id', $id)->orderBy('news.created_at', 'DESC')
            ->select('news.*', 'categories.title as category_title', 'categories.highlight_news_id as highlight_news_id')->paginate(12);
